  ...
  return (
    <>
      ...
      <main>
        <section className="relative overflow-hidden border-b border-border/60 bg-[#f8faf7]">
          ...
          <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-4 pb-16 pt-12 sm:px-6 sm:pb-24 sm:pt-20 lg:grid-cols-[0.9fr_1.1fr] lg:gap-4 lg:pt-24">
            <div className="max-w-xl">
              ...
              <p className="animate-fade-up mt-6 max-w-lg text-lg leading-8 text-muted">O Patrimo transforma gastos, reserva, metas e investimentos em uma visão simples para você decidir melhor — hoje e no futuro.</p>
        ...
        <section className="bg-surface py-20 sm:py-28"><div className="mx-auto grid max-w-6xl gap-12 px-4 sm:px-6 lg:grid-cols-[1fr_0.85fr] lg:items-center"><InteractiveDemo /><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-brand-700">Experimente agora</p><h2 className="mt-4 text-4xl font-extrabold leading-tight text-ink sm:text-5xl">Antes de organizar tudo, entenda o que importa.</h2><p className="mt-5 leading-7 text-muted">O Patrimo começa pela decisão, não pela planilha. Faça uma simulação e veja como uma pequena mudança mensal pode proteger sua reserva e acelerar uma meta.</p><div className="mt-8 rounded-2xl border border-border bg-card p-5"><div className="flex items-start gap-3"><Upload className="mt-0.5 h-5 w-5 text-brand" /><div><p className="font-bold text-ink">Importe seu extrato quando quiser</p><p className="mt-1 text-sm leading-6 text-muted">CSV brasileiro com prévia, categorias e edição. Open Finance fica marcado como <strong className="text-brand-700">em breve</strong>, sem prometer o que ainda não está pronto.</p></div></div></div></div></div></section>
        ...
        <section id="academia" className="mx-auto max-w-6xl px-4 pb-20 sm:px-6 lg:pb-28"><div className="rounded-[2rem] bg-brand-50 p-8 sm:p-12"><div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-brand-700">Aprenda fazendo</p><h2 className="mt-3 text-4xl font-extrabold text-ink">Academia Patrimo</h2><p className="mt-4 max-w-xl leading-7 text-muted">Conteúdo direto para transformar cada módulo em uma decisão prática. Aulas, calculadoras e checklists para acompanhar o seu ritmo.</p></div><ButtonLink href="/app/escola" variant="secondary">Conhecer a Academia <ArrowRight className="h-4 w-4" /></ButtonLink></div></div></section>
